...
if [ $# != 4 ]; then
   echo "Usage: scripts/mkgraph.sh  <tree> <model> <graphdir> <picdir>"
   exit 1;
fi
...
fsttablecompose data_toy/L_disambig.fst data_toy/G.fst | fstdeterminizestar --use-log=true | \
...
echo 
fstrandgen --select=log_prob $dir/LG.fst | fstprint --isymbols=data_toy/phones_disambig.txt --osymbols=data_toy/words.txt -

grep '#' data_toy/phones_disambig.txt | awk '{print $2}' > $dir/disambig_phones.list
...
fstcomposecontext $monophone_opts \
 ...
 # for debugging:
 fstmakecontextsyms data_toy/phones.txt $dir/ilabels > $dir/context_syms.txt
 echo "Example string from CLG.fst: "
 echo 
 fstrandgen --select=log_prob $dir/CLG.fst | fstprint --isymbols=$dir/context_syms.txt --osymbols=data_toy/words.txt -
...
fsttablecompose $dir/Ha.fst $dir/CLG2.fst | \
 tee $dir/HCLGa_0_raw.fst | fstdeterminizestar --use-log=true | \
 tee $dir/HCLGa_1_det.fst | fstrmsymbols $dir/disambig_tid.list | \
 tee $dir/HCLGa_2_rmaux.fst | fstrmepslocal | \
 tee $dir/HCLGa_3_rmeps.fst | fstminimizeencoded | \
 tee $dir/HCLGa_4_min.fst > $dir/HCLGa.fst

# Print the above HCLGa graphs in text (debug)
#fstprint $dir/HCLGa_0_raw.fst > $picdir/HCLGa_0_raw.fst.txt
#fstprint $dir/HCLGa_1_det.fst > $picdir/HCLGa_1_det.fst.txt
#fstprint $dir/HCLGa_2_rmaux.fst > $picdir/HCLGa_2_rmaux.fst.txt
#fstprint $dir/HCLGa_3_rmeps.fst > $picdir/HCLGa_3_rmeps.fst.txt
#fstprint $dir/HCLGa_4_min.fst > $picdir/HCLGa_4_min.fst.txt
...
fstrandgen --select=log_prob $dir/HCLG.fst |  fstprint --osymbols=data_toy/words.txt > $dir/rand.txt
cat $dir/rand.txt | awk 'BEGIN{printf("0  ");} {if(NF>=3 && $3 != 0){ printf ("%d ",$3); }} END {print ""; }' > $dir/rand_align.txt

show-alignments data_toy/phones.txt $model ark:$dir/rand_align.txt
